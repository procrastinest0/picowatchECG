...
class EPD_2in13_V4:
    ...
    def init(self):
        ...
        self._cmd(0x11, bytes([0x05]))
        self._cmd(0x44, bytes([0x00, 0x0F]))
        self._cmd(0x45, bytes([0xF9, 0x00, 0x00, 0x00]))
        self._cmd(0x3C, bytes([0x05]))
        self._cmd(0x21, bytes([0x00, 0x80]))
        self._cmd(0x18, bytes([0x80]))
        self._cmd(0x4E, bytes([0x00]))
        self._cmd(0x4F, bytes([0xF9, 0x00]))
        self._wait_busy()

    def init_partial(self):
        """Load partial refresh LUT for fast B/W-only updates."""
        self._cmd(0x3C, bytes([0x80]))
        self._cmd(0x32, _PARTIAL_LUT[:153])
        self._cmd(0x3F, bytes([_PARTIAL_LUT[153]]))
        self._cmd(0x03, bytes([_PARTIAL_LUT[154]]))
        self._cmd(0x04, bytes([_PARTIAL_LUT[155],
                               _PARTIAL_LUT[156],
                               _PARTIAL_LUT[157]]))
        self._cmd(0x2C, bytes([_PARTIAL_LUT[158]]))
        self._wait_busy()
